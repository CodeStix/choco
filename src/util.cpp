...
llvm::Value *generateMalloc(GenerationContext *context, llvm::Type *type)
{
    ...
    auto pointer = context->irBuilder->CreateCall(mallocFunction, parameters, "malloc");

    // Convert opaque pointer to typed pointer
    return context->irBuilder->CreateBitCast(pointer, llvm::PointerType::get(type, 0), "typedmalloc");
}
...
llvm::Value *generateFree(GenerationContext *context, llvm::Value *toFree)
{
    ...
    if (freeFunction == NULL)
    {
        ...
        freeFunction = llvm::Function::Create(functionType, llvm::Function::ExternalLinkage, "malloc", *context->module);
